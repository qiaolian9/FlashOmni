...
from .jit import FLASHOMNI_CSRC_DIR, has_prebuilt_ops, load_cuda_ops
    ...
    if _quantization_module is None:
        if has_prebuilt_ops:
            ...
            _quantization_module = load_cuda_ops(
                "quantization",
                [
                    FLASHOMNI_CSRC_DIR / "quantization.cu",
                    FLASHOMNI_CSRC_DIR / "flashomni_quantization_ops.cu",
                ],
            )
...
@register_custom_op("flashomni::packbits", mutates_args=())
...
@register_fake_op("flashomni::packbits")
...
def packbits(x: torch.Tensor, bitorder: str = "big") -> torch.Tensor:
    ...
    >>> import torch
    >>> from flashomni import packbits
