...
def get_cu_file_str(
    ...
    def get_insts(attention_variant, dtype_out):
        return "\n".join(
            [
                """template cudaError_t BatchSparseFAWithRaggedKVDispatched<{cta_tile_q}, {head_dim_qk}, {head_dim_vo}, {pos_encoding_mode}, {use_fp16_qk_reduction}, {mask_mode}, {attention_variant}, Params>(
    Params params,
    {dtype_out}* tmp_v,
    ...
    content = f"""#include <flashomni/attention_impl.cuh>

namespace flashomni {{

using Params = BatchRaggedSparseFAParams<{dtype_q}, {dtype_kv}, {dtype_out}, {idtype}>;

using AttentionVariant1 = DefaultAttention<{use_custom_mask}, /*use_logits_soft_cap=*/false, /*use_alibi_bias=*/false>;

{get_insts("AttentionVariant1", dtype_out)}

using AttentionVariant2 = DefaultAttention<{use_custom_mask}, /*use_logits_soft_cap=*/true, /*use_alibi_bias=*/false>;
