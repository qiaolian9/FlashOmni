#ifndef FLASHOMNI_GEMM_REDUC_CUH_
#define FLASHOMNI_GEMM_REDUC_CUH_
...
__global__ void flashomni_gemm_reduction_kernel(const T *Aptr, const T *Bptr,  T *bias_ptr,
    ...
    if (bias_ptr != nullptr) {
        if (!is_for_cached) {
            ...
            Tensor gBias = local_tile(Bias, make_tile(Int<BM>{}, Int<BN>{}), make_coord(iy, ix)); // (BM, BN) 
    ...
    // flashomni for gemm who is after attn
    uint32_t sparse_info_offset, sparse_info_offset_loaded = (uint32_t)-1;
    const uint32_t sparse_kv_info_offset_base = (num_text_tokens + iy * BM) / sparse_q_size * num_qo_heads;
...
#pragma unroll
    for (;itile_to_read < ntile && cur_cached_stages < kStage - 1;) {
        sparse_info_offset = sparse_kv_info_offset_base + itile_to_read * BK / head_dim;
...
#pragma unroll
    ...
    #pragma unroll
    for (int fake_commit = 0; fake_commit < kStage - 1 - cur_cached_stages; fake_commit ++) {
        cp_async_fence();
    }
...
template <typename T>
cudaError_t FLASHOMNIGEMMReductionRun(T* x, T* w, T* c, T* bias, 
                             uint32_t M, uint32_t N, uint32_t K, uint32_t sparse_q_size, 
                             uint32_t batch_size, uint32_t num_qo_heads, uint32_t head_dim,
                             uint8_t* sparse_info, uint32_t* sparse_info_indptr, 
    ...
    auto kernel = flashomni_gemm_reduction_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>;
    FLASHOMNI_CUDA_CALL(
